...
\begin{figure}[th]
\centering
\includegraphics[scale=0.9]{uvod/centralna_dogma}
...
\keyword{genetički kod} i opisuje preslikavanje uzastopnih trojki baza nukleinske
...
(originalna sekvenca). Skup homologih proteina čini \keyword{proteinsku
...
\begin{itemize}
  \item
    Dve sekvence su \keyword{ortologe} (orto - pravi) ako predstavljaju u osnovi istu
...
Centralni alfa ugljenikov atom ($C_{\alpha}$) povezan je sa \keyword{amino grupom} ($-NH_2$), 
\keyword{karboksilnom grupom} ($-COOH$), atomom vodonika i tzv. \keyword{R grupom}.
Vrsta aminokiseline određena je R grupom još poznatom po nazivu \keyword{bočni niz} ili \keyword{bočni ostatak} \en{residue}.
...
pp).
% Pod proteinom se podrazumeva polipeptid velike dužine. Različiti izvori
...
\ref{fig:AK_vene} od čega izdvajamo sledeću klasifikaciju najadekvatniju u
odnosu na domen istraživanja:
\begin{itemize}
  \item Nepolarne AK -
...
\centering
% \hspace*{-2.0cm} 
\includegraphics[scale=0.75]{uvod/aminokiselina.pdf}
...
\begin{figure}[th]
\centering
\includegraphics[scale=0.85]{uvod/AK_vene.pdf}
...
su obično sačinjeni od nekoliko elemenata sekundarne strukture koja logički
sledi nakon primarne, međutim tercijarna struktura je suštinska a sekundarna
šematska na osnovu nje.
...
\begin{figure}[th]
\centering
\includegraphics[scale=0.40]{uvod/sekundarna.pdf}
\caption {
...
naziva se \keyword{lokalno savijanje} \en{local folding}.
...
\en{short turn, long loop}. Specijalno, $\beta$-ukosnica \en{$\beta$-hairpin}
...
tačaksti dijagram \en{scatter plot} svih $\psi$ i $\phi$ vrednosti jednog polipeptida.

Pored gore navedenih, elementima sekundarne strukture takođe se smatraju
...
\keyword{konzervirani} ili očuvani.  Konzervirani elementi sekundarne strukture
domena predstavljaju \keyword{strukturne motive}, kraće \keyword{motive} i
...
izvršavaju dovoljno brzo ili uopšte ne vrše.  \keyword{Katalizator} je molekul
koji ubrzava (olakšava) hemijsku reakciju a da se sam pri tome ne menja, pa
može ponovo da se koristi.  \keyword{Enzim} je katalizator biološkog porekla,
najčešće protein.  Molekul koji biva \keyword{katalizovan} odnosno promenjen u
interakciji sa enizmom zovemo \keyword{supstrat}. Mesto enzima koje interaguje
sa supstratom naziva se \keyword{aktivni region}.  Skoro svaka reakcija u živom
organizmu ubrzana je enzimskom katalizom. Ime enzima obično se završava sa
'-za' \en{-ase}.  Karakteristika enzima je da katalizuje reakciju specifičnog
molekula.

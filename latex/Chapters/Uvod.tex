...
Slike \ref{fig:kod} zove se \keyword{genetički kod} i opisuje mapiranje nukleinskih
trojki tzv. \keyword{kodona} u aminokiseline ili stop oznaku.
...
koja biva transkribovana u RNK.  Neki geni su tzv.  \keyword{nekodirajući geni} i
...
\keyword{genom}.  Svi transkribovani RNK molekuli čine \keyword{transkriptom},
...
Vrsta aminokiseline određena je R grupom još poznatom kao \keyword{bočni niz} ili \keyword{bočni ostatk} \en{residue}.
...
tzv. \keyword{pp. lanac} ili \keyword{kičma peptida} sa koje štrče bočni nizovi.
...
Protein je sačinjen iz jednog (monomerni) ili više (oligomerni) polipeptdinih lanaca.
...
odnosno redosledom aminokiselina. Primarna struktura predstvalja \keyword{sekvencu
...
\begin{itemize}
  \item \keyword{$\alpha$-spirala} - je spiralna struktura u kojoj R grupe štrče spolja
  \item \keyword{$\beta$-traka}  - predstavlja spoj dva ili više (anti)paralelnih delova pp. lanca.
\end{itemize}
...
petlje \en{short turn, long loop}. Specijalno zavijutak \en{$\beta$-hairpin} je
kratak zaokret lanca kod antiparalelne $\beta$-traka.
Formiranje sekundarne strukture naziva se \keyword{lokalno savijanje}.
...
\keyword{savijanje} \en{folding}.  Pod poznavanjem tercijarne strukture misli
...
\keyword{ Kvaterniorna struktura } opisuje proteinske komplekse (oligomerne proteine) sastavljene
...
sličnošću domena, još kažemo da su domeni \keyword{konzervirani} ili očuvani.
Konzervirani elementi sekundarne strukture domena predstavljaju
\keyword{strukturne motife}, kraće \keyword{motif} i okarakterisani su velikom

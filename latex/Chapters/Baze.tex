...
nastao je sa ciljem da  objedini biologiju pod jedan univerzum termina za opis
genskih proizvoda svih vrsta organizama \parencite{GO2000}.

\subsection{GO termin}

GO termin može biti zastareo i tada se relacijom \keyword{replaced\_by}
pokazuje na noviji termin. Relacija \keyword{consider} ukazuju na
postojanje mogućih ekvivalentnih termina. Pored glavnog univerzuma termina postoje i
podskupovi\footnote{Uglavnom ovi podskupovi predstavljaju model organizme}
termina tzv. \textit{GO slim}. U donjem desnom delu Slike
\ref{fig:kinase} prikazani su \textit{GO slim} podskupovi.
...
drugim veb stranam i bazama podataka često vezanih uz definiciju.  Uz GO termin
...
Jedan od najčešće korišćenih formata je  ravni \file{.obo} formata, a pored njega
...
Funkcija genskog produkta je posao koji obavlja ili "osobinu" koju ima.
...
utovarivati prtljag niti stiskati kvačilo.  Najbitnije karakteristike
molekulske funkcije su:\parencite{go_mf}
\begin{itemize}
...
\begin{itemize}
  \item \keyword{$x$ binding} - interreaguje selektivno i nekovalentno sa $x$
...
\begin{enumerate}
  \item \keyword{\swissprot}  sadrži visoko kvalitetne anotacije
    \keyword{neredundantnih} ( objašnjeno u produžetku stavka\ref{red}) proteinskih sekvenci.
...
\begin{enumerate}
  ...
  \item Postojnost proteina \keyword{PE} \en{Protein existance} opisuje stepen
    sigurnosti da protein postoji \ref{fig:PE}. Moguće vrednosti u rastućem poretku su:

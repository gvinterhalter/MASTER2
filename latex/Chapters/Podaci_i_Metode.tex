...
klasterovanja je da svaki protein dobiva težinu kojom doprinosi daljoj analizi.
...
Dalja analiza  izvršena je pod pretpostavkom da CAFA3 skup nije statistički redundantan.
Ova pretpostavka uporošćava računske metode analize.
...
Naša analiza primarno je orijentisna ka korišćenju GO termina za opis funkcije
što je razlikuje od originalnog pristupa korišćenja ključnih reči.  Analiza GO
termina zahteva poznavanje prvenstveno \textit{IS\_A} roditeljske veze između
termina. Takođe, tokom istraživanja bile su nam potrebne i ostale informacije o
terminima. Pomenute informacije dobili smo iz datoteke \cite{go_obo} verzije
01.12.2017.
...
antoacijama ključnih reči, broju ključnih reči ali i sadržaju proteinskih
sekvenci.  Radi procene uticaja navedenih razlika na rezulat, odlučeno je da se
...
je bilo neophodno mapirati ih nazad na \swissprot proteine čije anotacije ka
ključnim rečima su poznate. Ovaj korak objedinjavanja baza podataka opisan je u
...
Nažalost, \textbf{ograničenost} današnjih podataka i razvijenih metoda su brojna:
\begin{itemize}
  \item
    Baza eksperimentalno utvrđenih neuređenih regiona
    \textit{\keyword{DisProt}} ima svega 803 proteina sa opisanih 2167
    neuređenih regiona \parencite{Piovesan2016}.  Još gore, kvalitet navedenih
    informacija je diskutabilna zbog razlika u pouzdanosti među
  ...
  \item
    Prediktori se terniraju na malom podskupu proteina iz \textit{DisProt}
...
\begin{mydef}
\label{pdis_def}
Protein je \keyword{putativno neuređen} (najverovatnije neuređen, u daljem tekstu neuređen) \en {putatively disordered}
...
aminokiselina javlja sa istom verovatnoćom, odnosno $p=1/20$. U statistici prvi
model je još poznato kao model jednakih verovatnoća \en{equiprobable model
...
% \decoRule
\caption{Očekivana i izmerena (kod kičmenjaka) učestalost aminokiselina (Preuzeto sa \cite{AA_freq_link}).  \footnotesize
  Očekivana učestalost kodona izračunata je kao proizvod učestalosti nukleinskih baza ($A:0.303, C:0.217, G:0.261, U:0.2$) koje ga čine.
...
ako je p-vrednost \en{p-value} $<0.05$ onda je funkcija $j$ povezana sa neuređenim
proteinima, a ako je \textit{p-value} $>0.95$ onda je funkcija $j$ povezana sa

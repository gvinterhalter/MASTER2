...
referentni rezultati izraženi u terminima \keyword{ključnih reči} dok su ovde
rezultati izraženi u \keyword{GO-terminima}. Oba pristupa proizvode listu
funkcija, ali GO-termini zbog granularnosti se prirodnije predstavljaju
grafovski i sadrže mnogo više funkcija. Jedan od rezultata ovog istraživanja
predstavlja poređenje ova dva pristupa.
...
CAFA3 trening skup (u nastavku samo CAFA3 skup ili CAFA3 podaci) je podskup baze \swissprot (iz 2016.) koji
...
Za razliku od proteina u bazi \swissprot čije je postojanje pretežno utvrđeno
...
\swissprot proteini su kodirani koristeći \keyword{IUPAC}
...
\keyword{validnom proteinskom sekvencom} smatraćemo sekvencu koja predstavlja
...
\begin{enumerate}
  \item \file{uniprot\_sprot\_exp.fasta}  sadrži 66 841 protein od kojih 66 599
    predstavljaju validnu proteinsku sekvencu za našu analizu. Od opisivanih
...
Analiza GO-termina zahteva poznavanje prvenstveno  roditeljske veze
\textit{is\_a} između termina. Takođe, tokom istraživanja bile su nam potrebne
i ostale informacije o terminima. Pomenute informacije dobili smo iz datoteke
\file{go.obo} \cite{go_obo} verzije 01.12.2017.

Odnos tj. preslikavanje između ključnih reči i GO-termina dostupno je sa dva izvora:
\begin{itemize}
  \item \file{keywlist.txt}\cite{keywlist_txt} verzija 20.12.2017 sadrži
    ...
    \keyword{molekulskih funkcija}. Više o preslikavanju biće
...
je bilo neophodno preslikati ih nazad na \swissprot proteine čije su anotacije 
...
iskorišćene za proveru validnosti preslikavanja ključnih reči na GO-termine.
...
na uređene proteine podrazumeva da funkcija zavisi više od neuređenih regiona nego uređenih domena. 
...
Referentni autori koristili su \keyword{PONDR VL3E} prediktor koji
...
Relevantne karakteristike prediktor VSL2b detaljno su opisane u \ref{VSL2b}.
Za potrebe analize referentni autori  uvode sledeću definiciju:

\begin{definicija}
\label{pdis_def}
Protein je \keyword{putativno neuređen} (najverovatnije neuređen, u daljem
tekstu \keyword{neuređen}) \en{putatively disordered} ako sadrži bar jedan region veći
ili jednak od 40 uzastopnih aminokiselina takvih da im je predviđena
neuređenost iznad 0.5. 
\end{definicija}
...
Uslov ''$\ge40$'' iz referentnog rada delom je posledica ograničenja VL3
...
raste sa porastom njegove dužine. Ovo utiče na statističku značajnost
...
sadrži značajno mnogo predviđenih (ne)uređenih proteina.
...
\begin{definicija}
  ...
  Funkcija (GO-termin ili ključna reč) je neuređena ako je povezana ($p<0.05$) sa
  neuređenim proteinima, a uređena ako je povezana ($p>0.95$) sa uređenim proteinima.
\end{definicija}
